...
function calc(el) {
    ...
    let spin = Math.abs(checkValidInput(document.getElementById('spin').value));
    const spin_type_el = document.getElementById('spin_type');
    const spin_type = spin_type_el ? Number(spin_type_el.value) : 1;

    if (spin_type === 0)
        spin = 0.0;
    else
        spin *= spin_type; // +1 Backspin, -1 Frontspin
    ...
    if (f[index_f].power != -1) {
        ...
        result.color = 'White';
        const lateralYards = desvioByDegree(f[index_f].desvio, distance);
        const aim = getAimConversion(lateralYards);
        const desvioSquares = aim.value;
        const direcaoTiles = Math.abs(desvioSquares) < 0.01 ? 'CENTRO' : (desvioSquares > 0 ? 'ESQUERDA' : 'DIREITA');
        const setaTiles = direcaoTiles === 'ESQUERDA' ? '←' : (direcaoTiles === 'DIREITA' ? '→' : '•');

        const rawForceYards = (f[index_f].power_range * f[index_f].power);
        const forceCorrection = getFieldForceCorrection(input_values);
        const forceYards = rawForceYards + forceCorrection;
        const s4Band = getS4PowerBand(forceYards, f[index_f].power_range);

        if (forceYards > f[index_f].power_range + 0.01) {
            result.color = '#fbbf24';
            result.innerHTML = `NÃO ALCANÇA &nbsp; | &nbsp; PRECISA: ${forceYards.toFixed(1)}y &nbsp; | &nbsp; MÁXIMO ATUAL: ${f[index_f].power_range.toFixed(1)}y`;
            return result.innerHTML;
        }

        const spinName = input_values.spin > 0 ? 'BACK' : (input_values.spin < 0 ? 'FRONT' : 'SEM SPIN');
        result.innerHTML =
            `FORÇA: <strong>${forceYards.toFixed(1)}y</strong> &nbsp; | &nbsp; ` +
            `MIRA: <strong>${setaTiles} ${Math.abs(desvioSquares).toFixed(2)} quadradinhos</strong> (${direcaoTiles})` +
            `<br><span style="font-size:.72em;color:#94a3b8">` +
            `S4: ${s4Band.lower.toFixed(1)}–${s4Band.upper.toFixed(1)}y · ` +
            `desvio físico ${Math.abs(lateralYards).toFixed(3)}y · ${aim.label} · ${spinName} ${Math.abs(input_values.spin).toFixed(1)}` +
            `</span>`;
		return result.innerHTML;
		
    }else {
        result.color = 'darkRed'
        result.innerHTML = 'Power is too weak!.'
		ret
    }
}
...
// V6 aiming model.
// desvioByDegree() is a physical lateral displacement in yards.
// There is no universal "square" size in PangYa: old green grids and modern
// SmartView use different scales. Keep every conversion explicit.
const SMART_YARDS_TO_PB = 0.21656;
const SMART_DISPLAY_FACTOR = 0.8476487854;

function getAimConversion(lateralYards) {
    const modeEl = document.getElementById('aim_mode');
    const mode = modeEl ? modeEl.value : 'field800';

    if (mode === 'green15')
        return { value: lateralYards / 1.5, label: 'grade 1,5y' };

    if (mode === 'green20')
        return { value: lateralYards / 2.0, label: 'grade 2,0y' };

    if (mode === 'smart800') {
        // Eternal/Acrisio current Smart Calculator:
        // 800x600 PB pequeno => dis=1.25; default aim conversion=4;
        // SmartView is 1/4 of the maximum-zoom floor movement.
        const pb = lateralYards / SMART_YARDS_TO_PB;
        const floorSquares = (pb * 1.25) * SMART_DISPLAY_FACTOR / 4.0;
        return { value: floorSquares * 0.25, label: 'SmartView 800x600' };
    }

    // User field calibration, intentionally editable.
    const customEl = document.getElementById('square_yards');
    let yardsPerSquare = customEl ? checkValidInput(customEl.value) : 1.25;
    if (!(yardsPerSquare > 0))
        yardsPerSquare = 1.25;

    return { value: lateralYards / yardsPerSquare, label: 'calibração 800x600' };
}

function getS4PowerBand(forceYards, powerRange) {
    // Current Acrisio-derived Smart Calculator resolves old-bar force on a
    // 1/360 power fraction. Show the enclosing interval instead of pretending
    // a continuous mathematical value is directly executable on the S4 bar.
    const step = powerRange / 360.0;
    if (!(step > 0))
        return { lower: forceYards, upper: forceYards, step: 0 };

    const lower = Math.floor(forceYards / step) * step;
    const upper = Math.ceil(forceYards / step) * step;
    return { lower, upper, step };
}

function getFieldForceCorrection(input_values) {
    // The former -2y correction was based on a shot whose back/front spin
    // direction had not been recorded. Keep it disabled until that anchor is
...
//Adding this by ELITEC
function processFormData(data) {
    const parsedData = JSON.parse(data);
    console.log("Received data:", parsedData);

    // Perform calculations or other tasks with the parsed data
}
