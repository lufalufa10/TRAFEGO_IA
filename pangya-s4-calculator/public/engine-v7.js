...
function calc(el) {
    ...
    if (f[index_f].power != -1) {
        ...
        result.color = 'White';
        const lateralS4 = desvioByDegree(f[index_f].desvio, distance);
        const desvioSquares = getS4AimTiles(lateralS4);
        ...
        result.innerHTML =
            ...
            `escala S4 HWI · desvio ${Math.abs(lateralS4).toFixed(3)} · ${spinName} ${Math.abs(input_values.spin).toFixed(1)}` +
            `</span>`;
		return result.innerHTML;
...
// V7 Season 4 aiming model.
//
// IMPORTANT:
// Acrisio's desvioByDegree() output is already on the same practical scale
// used by classic Season 4 HWI guides for "tiles/quadradinhos".
// Benchmark: classic HWI example (1W 264, Toma, 231y, -3.1m, wind 5 @ 30°)
// gives 1.415 tiles; this physics engine gives ~1.45 before field tuning.
// Do NOT divide this value by an arbitrary visual-square constant.
function getS4AimTiles(lateralValue) {
    return lateralValue;
}
