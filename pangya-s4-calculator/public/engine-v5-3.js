...
function calc(el) {
    ...
    if (f[index_f].power != -1) {
        ...
        const desvioSquares = lateralYards / FIELD_SQUARE_YARDS;
        ...
        result.innerHTML = `FORÇA: ${forceYards.toFixed(1)}y &nbsp; | &nbsp; MIRA: ${setaTiles} ${Math.abs(desvioSquares).toFixed(2)} quadradinhos (${direcaoTiles})`;
		return result.innerHTML;
...
// Field calibration for Lucas's Season 4 reference.
// The physics engine outputs lateral deviation in yards, not visual grid squares.
// Empirical reference #1: 0.63 lateral yards ≈ 0.50 visual square.
const FIELD_SQUARE_YARDS = 1.25;

// Force calibration is intentionally narrow until more real shots are collected.
// Reference #1: 1W Toma +10y, 239y, -9.45m, wind 1 @ 90°, 100%, spin 3:
// engine 232.4y; field reference ≈ 230.4y.
function getFieldForceCorrection(input_values) {
    if (input_values.club_info === CLUB_INFO._1W
        && input_values.shot === SHOT_TYPE.TOMAHAWK
        && input_values.power_shot === POWER_SHOT_FACTORY.ONE_POWER_SHOT
        && Math.abs(input_values.spin - 3.0) < 0.01)
        return -2.0;
