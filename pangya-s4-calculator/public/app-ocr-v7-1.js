(() => {
  ...
  function guessS4PixelDigit(region, comp) {
    const holes=holeInfo(region,comp);
    if(holes.length>=2)return '8';

    const ratio=comp.w/Math.max(1,comp.h);
    ...
    // PangYa S4 red distance digits behave like a stylized seven-segment font.
    // Measure ink occupancy in seven regions and choose the closest glyph.
    const boxes={
      t:[.00,.22,.20,.80],
      m:[.38,.62,.15,.85],
      b:[.78,1.00,.20,.80],
    ...
    for(const [k,b] of Object.entries(boxes)){
      const yy0=Math.floor(comp.h*b[0]), yy1=Math.max(yy0+1,Math.ceil(comp.h*b[1]));
      const xx0=Math.floor(comp.w*b[2]), xx1=Math.max(xx0+1,Math.ceil(comp.w*b[3]));
      let on=0,total=0;
      for(let yy=yy0;yy<yy1;yy++)for(let xx=xx0;xx<xx1;xx++){
        total++;
        if(region.mask[(comp.y+yy)*region.w+(comp.x+xx)])on++;
      }
      feat[k]=on/Math.max(1,total);
    }

    const patterns={
      '2':{t:1,ur:1,m:1,ll:1,b:1,ul:0,lr:0},
      '3':{t:1,ur:1,m:1,lr:1,b:1,ul:0,ll:0},
      '4':{ul:1,ur:1,m:1,lr:1,t:0,b:0,ll:0},
      '5':{t:1,ul:1,m:1,lr:1,b:1,ur:0,ll:0},
      '7':{t:1,ur:1,lr:1,m:0,b:0,ul:0,ll:0}
    };
    ...
    for(const [digit,pat] of Object.entries(patterns)){
      ...
      for(const k of Object.keys(boxes)){
        const target=pat[k] ? .68 : .22;
        const weight=(k==='ll'||k==='lr')?1.35:1.0;
        score+=weight*Math.pow(feat[k]-target,2);
      }
  ...
  async function readDistanceFromHud(worker) {
    ...
    const comps=r.comps
      .filter(x=>x.area>=18&&x.h>=8&&x.w/x.h<1.05)
      .sort((a,b)=>a.x-b.x);
    const digits=[];
    for(const comp of comps){
      if(digits.length>=3)break;
      // Ignore the final "y": distance has at most three numeric glyphs.
      const d=guessS4PixelDigit(r,comp);
      if(d)digits.push(d);
    }
  ...
  async function readHeightFromHud(worker) {
    // Raw upscaled crop works better than adaptive threshold on the outlined S4 font.
    // Verified against the supplied 640x480 screenshot: "-7.53m".
    const x0=Math.max(0,Math.floor(canvas.width*.510));
    const y0=Math.max(0,Math.floor(canvas.height*.045));
    const x1=Math.min(canvas.width,Math.ceil(canvas.width*.615));
    const y1=Math.min(canvas.height,Math.ceil(canvas.height*.105));
    const w=Math.max(1,x1-x0),h=Math.max(1,y1-y0);

    const crop=document.createElement('canvas');
    crop.width=w*12;crop.height=h*12;
    const cc=crop.getContext('2d');
    cc.imageSmoothingEnabled=true;
    cc.fillStyle='white';cc.fillRect(0,0,crop.width,crop.height);
    cc.drawImage(canvas,x0,y0,w,h,0,0,crop.width,crop.height);
    ...
    let value=null;
    if(matches.length){
      // Use the last decimal token; HUD decorations can create stray marks before it.
