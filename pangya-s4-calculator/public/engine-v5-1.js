...
function calc(el) {
    ...
    if (f[index_f].power != -1) {
        ...
        const forceYards = (f[index_f].power_range * f[index_f].power);
        if (f[index_f].power > 1.0001) {
            result.color = '#fbbf24';
            result.innerHTML = `NÃO ALCANÇA &nbsp; | &nbsp; PRECISA: ${forceYards.toFixed(1)}y &nbsp; | &nbsp; MÁXIMO ATUAL: ${f[index_f].power_range.toFixed(1)}y`;
            return result.innerHTML;
        }
        result.innerHTML = `FORÇA: ${forceYards.toFixed(1)}y &nbsp; | &nbsp; MIRA: ${setaTiles} ${Math.abs(desvioTiles).toFixed(2)} quadradinhos (${direcaoTiles})`;
		return result.innerHTML;
